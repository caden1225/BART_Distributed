...
colossalai.launch_from_torch(config='/data/projects/BART_Distributed/config.py')
...
def set_args():
    ...
    parser.add_argument('--log_steps', default=5, type=int, required=False, help='xunlianguochengshuchurizhidebuchang')
...
logger.info(f"start training with total step is {total_steps}")
best_val_loss = 10000
current_step = 0
...
for epoch in range(gpc.config.NUM_EPOCHS):
    ...
    for batch in train_loader:
        ...
        labels = batch[1].cuda()
        decoder_input_ids = shift_tokens_right(labels, pad_token_id=args.pad_token_id, decoder_start_token_id=args.sep_token_id)
        ...
        if gpc.get_global_rank() == 0:
            ...
            if current_step % args.log_steps == 0:
                logger.info(f"#training### step {current_step} in epoch {epoch}: train_loss is {train_loss:.5}, nnl_loss is {nll_loss:.5} #############", ranks=[0])
    ...
    if gpc.get_global_rank() == 0:
        logger.info("############# epoch {}: train average loss is {} #############".format(epoch, avg_loss), ranks=[0])
